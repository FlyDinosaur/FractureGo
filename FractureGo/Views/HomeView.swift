...
struct HomeView: View {
    ...
    let trainings = [
        Training(
            ...
            arrowColor: "f19ca2",
            destination: AnyView(HandLevelView())
        ),
        Training(
            title: "手臂训练", 
            backgroundImage: "Arm",
            ...
            arrowColor: "9ecd57",
            destination: AnyView(ArmLevelView())
        ),
        Training(
            title: "腿部训练",
            backgroundImage: "Leg",
            ...
            arrowColor: "9f98fb",
            destination: AnyView(LegLevelView())
        )
    ]
            ...
            VStack(spacing: 0) {
                ...
                TabView(selection: $currentIndex) {
                    ForEach(trainings.indices, id: \.self) { index in
                        TrainingCardView(training: trainings[index])
                            .tag(index)
                    }
                }
...
struct TrainingCardView: View {
    ...
    var body: some View {
        ...
        .background(
            NavigationLink(destination: training.destination, isActive: $isNavigating) {
                EmptyView()
            }
            .opacity(0)
        )
